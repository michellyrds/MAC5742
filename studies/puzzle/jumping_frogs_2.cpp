...
bool blue_frog_can_jump(JumpingFrogsPuzzle* puzzle, int thread_id){
    if (thread_id < puzzle->size - 1 && puzzle->stones[thread_id + 1] == 1 &&
            puzzle->stones[thread_id + 2] == 0) return true;
    
    if (thread_id < puzzle->size && puzzle->stones[thread_id + 1] == 0) return true;
...
bool green_frog_can_jump(JumpingFrogsPuzzle* puzzle, int thread_id){
    int pos = puzzle->size + 1 + thread_id;

    if (pos > puzzle->size + 1 && puzzle->stones[pos - 1] == 2 &&
        puzzle->stones[pos - 2] == 0) return true;

    if (pos > puzzle->size && puzzle->stones[pos - 1] == 0) return true;
...
void* blue_frog_try_jump(void* arg) {
    ...
    while (true) {
        while(!blue_frog_can_jump(puzzle, thread_id)){
            pthread_cond_wait(&puzzle->can_jump, &puzzle->lock);
        }

        pthread_mutex_lock(&puzzle->lock);
        blue_frog_jump(puzzle, thread_id);
        check_deadlock(puzzle);
        pthread_cond_broadcast(&puzzle->can_jump);
...
void* green_frog_try_jump(void* arg) {
    ...
    while (true) {
        while(!green_frog_can_jump(puzzle, thread_id)){
            pthread_cond_wait(&puzzle->can_jump, &puzzle->lock);
        }
...
int main(int argc, char** argv) {
    ...
    pthread_cond_init(&puzzle.can_jump, NULL);
    ...
    pthread_cond_destroy(&puzzle.can_jump);
